		...
		for S in nx.connected_component_subgraphs(pw_subgraph):
			if S.number_of_nodes() >= 5:
				...
				s_count += 1
	 			...
	 			set1 = set()
	 			for node in S.nodes():
	 				for neibor_node in nx.all_neighbors(pw2G[pw],node):
	 					set1.add(neibor_node)	 
	 					b.append(neibor_node)
	 			#error = ['89941','9005-79-2','57530','646821','646048','648921','345456','375189','79837','318','111','29119']
	 			for node in set1:
	 				#if b.count(node)	>= S.number_of_nodes()/2.0 and node not in S.nodes() and node not in error:
	 				if b.count(node)	>= 3*S.number_of_nodes()/4.0 and node not in S.nodes():
	 					S.add_node(node)                          
	 					#print node     
				subnets[pw + '_' + str(s_count)] = S
				s_count += 1
...
def get_subnet_scores (subnets, weights_1, weights_2, num_samples):
	...
	for s_name in subnets:
		...
		for g in S_nodes:
			try:
	 			mean_weights_1[g] = np.mean(weights_1[g]) 	 				
	 			mean_weights_2[g] = np.mean(weights_2[g]) 
			except:
	 			subnets[s_name].remove_node(g) 
	 			print g,'ok' 
		S_nodes = subnets[s_name].nodes()
	...
	for s_name in subnets:
		...
		sub_density[s_name] = nx.density(subnets[s_name]) 
		for j in range(num_samples):
			scores_1[s_name][j] = sum(mean_weights_1[g] * weights_1[g][j] for g in S_nodes)
			scores_2[s_name][j] = sum(mean_weights_2[g] * weights_1[g][j] for g in S_nodes)
	#print scores_1['Integrin Signaling_0'][1] 
	#print scores_2['Integrin Signaling_0'][1]   
	'''	
	...
	for s_name in subnets:
		...
		if math.log(nx.density(subnets[s_name])) == 0 :
			subnet_scores[s_name] =  100*mean_x/ (std_err_x + epsilon)   
		else:
			subnet_scores[s_name] = -mean_x/ ((std_err_x + epsilon)*math.log(nx.density(subnets[s_name])))
	
		#subnet_scores[s_name] = mean_x/ (std_err_x + epsilon)
		#subnet_scores[s_name] = mean_x*pow(nx.density(subnets[s_name]),2)/ (std_err_x + epsilon)
		#subnet_scores[s_name] = mean_x*math.log(nx.density(subnets[s_name]))/ (std_err_x + epsilon)


        
		#subnet_scores[s_name] = mean_x*nx.density(subnets[s_name])/ (std_err_x + epsilon)
		#print s_name,':',round(mean_x/ (std_err_x + epsilon),3)
		#print round(nx.density(subnets[s_name]),3),round(math.log(nx.density(subnets[s_name])),3),round(subnet_scores[s_name],3)        
	print 'ok!!!!!!!!'
...
def pfsnet_core(Xc, Xt, X_genes, pw2G, beta, theta_1, theta_2):
	...
	print 'chayi genes control&test!!!'
	print len(he_genes_c)
	print len(he_genes_t)
    
	# extract subnets
	subnets_c = generate_subnets(pw2G, he_genes_c)
	subnets_t = generate_subnets(pw2G, he_genes_t)
	print len(subnets_c),'sub_c'
	print len(subnets_t),'sub_t'
...
def write_to_file_genes (f_path, subnets, sgnf_subnets):
...
def write_to_file_top10 (f_path, subnets,subnet_score):
	...
	fo = open(f_path, 'wb')
	#fo.write('subnet,density,exp_density,score\n')
	fo.write('subnet,score,genes\n')
	sorted_score = sorted(subnet_score.items(), key=operator.itemgetter(1),reverse=True)
    #log  <0
	#sorted_score = sorted(subnet_score.items(), key=operator.itemgetter(1))
	for snet in sorted_score:
		s_genes = ','.join(subnets[snet[0]].nodes())
		#print snet
		#if subnet_score[snet]>0:
			#fo.write(snet + ','+str(round(nx.density(subnets[snet]),3))+','+str(round(math.exp(nx.density(subnets[snet])),3))+',' + str(round(subnet_score[snet],3))+'\n')
		if snet[1]>0:
			#fo.write(snet[0] + ','+ str(round(nx.density(subnets[snet[0]]),3))+','+str(round(snet[1],3))+'\n')
			fo.write(snet[0] + ','+str(round(snet[1],3))+','+ s_genes+ '\n')        
	fo.close()
...
def main(argv):
	...
	# pfsnet
	subnets_c, subnets_t, subnet_scores_c, subnet_scores_t = pfsnet(Xc, Xt, X_genes, pw2G, beta, theta_1, theta_2, n_permutations)
	write_to_file_top10('arm_moregenes3/significant_subnetworks_control.csv', subnets_c,subnet_scores_c)
	write_to_file_top10('arm_moregenes3/significant_subnetworks_test.csv', subnets_t,subnet_scores_t)
	...
	# print results
	#write_to_file('pfsnet_results_Arm/significant_subnetworks_control.csv', subnets_c, sgnf_subnets_c)
	#write_to_file('pfsnet_results_Arm/significant_subnetworks_test.csv', subnets_t, sgnf_subnets_t)
